import { getProviderConfig, getCategories, getAutomationSettings, getCustomAiProviders } from "./db.js";
import { callGoogle } from "./ai-providers/google.js";
import { callOpenAICompatible } from "./ai-providers/openai-compatible.js";

function buildSystemPrompt(categoryNames, minWordCount) {
...
- Write a complete article of at least ${minWordCount} words (ideally 5-6 paragraphs, roughly
  350-500 words if the source material supports it) — this is a strict MINIMUM, not a rough
  suggestion. Structure it as:
...
- If the source text itself is thin, do NOT write a short article — instead reach the minimum
  word count by adding more context, background, and explanation of significance (see the next
  rule), never by inventing new facts, padding with filler, or repeating the same point in
  different words.
...
// Voorkomt dat één hangende provider (netwerkprobleem, provider die
// technisch niet netjes faalt maar gewoon nooit antwoordt) de hele
// generatie blokkeert — zonder dit kon callWithFallback in het slechtste
// geval minutenlang wachten vóórdat de volgende provider geprobeerd werd.
// 60s omdat een volledig artikel (350-500 woorden + claims-extractie in
// gestructureerde JSON) op de gratis lagen van Gemini/Groq soms ruim boven
// de 25s kan duren — een te kort getimede timeout forceert dan onterecht
// een fallback naar een zwakker model, wat precies averechts werkt op
// artikelkwaliteit (zo ontdekt: een artikel viel terug tot op Mistral,
// het laatste in de rij, en leverde een veel te kort artikel op).
...
// Bewust in-memory (niet in db.json) — een herstart/redeploy is een prima
// natuurlijk moment om elke cooldown te laten vervallen, in plaats van een
// oude quota-fout voor altijd te blijven onthouden.
const providerCooldowns = new Map(); // providerId -> tijdstip (ms) tot wanneer overslaan
const QUOTA_COOLDOWN_MS = 15 * 60 * 1000;

// Onderscheidt een quota-/rate-limit-fout (waarbij opnieuw proberen zinloos
// is totdat de tijd is verstreken) van elke andere fout (netwerkstoring,
// verkeerd modelnaam, etc.), waarbij WEL bij elke aanroep opnieuw geprobeerd
// moet worden — een verkeerd modelnaam die net gecorrigeerd is, mag niet
// per ongeluk nog 15 minuten overgeslagen blijven worden.
function isQuotaOrRateLimitError(err) {
  const msg = (err.message || "").toLowerCase();
  return msg.includes("429") || msg.includes("quota") || msg.includes("rate limit") || msg.includes("too many requests");
}

// Alleen actieve (nog niet verstreken) cooldowns, voor weergave in
// Instellingen → AI — puur informatief.
export function getProviderCooldowns() {
  const now = Date.now();
  const result = {};
  for (const [id, until] of providerCooldowns) {
    if (until > now) result[id] = new Date(until).toISOString();
  }
  return result;
}

// Standaard blijft Gemini eerst (bewust zo gekozen vanwege kwaliteit) —
// alleen als "Prioritize speed" aanstaat in Instellingen → Automatisering
// wordt Groq (de snelste van de geconfigureerde gratis providers) als
// eerste geprobeerd, de rest behoudt zijn onderlinge volgorde.
// Voegt de door de admin zelf toegevoegde providers (Instellingen → AI →
// "Add custom provider") toe aan de vijf ingebouwde — zo'n provider wordt
// via dezelfde generieke OpenAI-compatibele aanroep bediend als
// Groq/OpenRouter/Cerebras/Mistral al gebruiken, dus geen nieuwe
// providercode nodig om een nieuwe (OpenAI-compatibele) AI uit te
// proberen. Wordt bij elke generatie opnieuw opgebouwd (niet gecachet),
// zodat een net toegevoegde provider zonder herstart al meedoet.
function getAllProviders() {
  const custom = getCustomAiProviders().map((cp) => ({
    id: cp.id,
    label: cp.label,
    call: (cfg, systemPrompt, userPrompt) =>
      callOpenAICompatible({
        baseUrl: cp.base_url,
        apiKey: cfg.api_key,
        model: cfg.model || cp.default_model || "",
        systemPrompt,
        userPrompt,
      }),
  }));
...
function getOrderedProviders() {
  ...
  const { prioritize_speed } = getAutomationSettings();
  if (!prioritize_speed) return all;
...
async function callWithFallback(systemPrompt, userPrompt) {
  ...
  for (const provider of getOrderedProviders()) {
    ...
    if (cooldownUntil && cooldownUntil > now) {
      ...
      attempted.push(`${provider.label}: skipped — in cooldown after an earlier quota/rate-limit error (~${minutesLeft} min left)`);
      continue;
    }

    try {
      const rawText = await withTimeout(provider.call(cfg, systemPrompt, userPrompt), PROVIDER_TIMEOUT_MS);
      // Een geslaagde aanroep heft een eventuele oude cooldown meteen op —
      // bijv. als de admin net zelf een geldige sleutel/model heeft
      // ingesteld, hoeft er niet nog 15 minuten gewacht te worden.
      providerCooldowns.delete(provider.id);
      // failedProviders wordt meegegeven zodat een succesvolle generatie na
      // 1+ mislukte pogingen zichtbaar blijft voor de redacteur (zie
      // ai_provider_notes bij generateDraft) — zonder dit was de enige
      // aanwijzing dat er iets misging de servertlogs, die je normaal
      // nooit inziet.
      return { rawText, providerLabel: provider.label, failedProviders: attempted };
    } catch (err) {
      if (isQuotaOrRateLimitError(err)) {
        providerCooldowns.set(provider.id, Date.now() + QUOTA_COOLDOWN_MS);
        attempted.push(`${provider.label}: ${err.message} (now skipped for ~15 min)`);
      } else {
...
// Test een specifieke provider los van de normale generatie-terugvalketen
// — precies voor het geval dat de gebruiker beschrijft: "ik kom een AI
// tegen en wil 'm even uitproberen" zonder te hoeven wachten tot de
// andere providers toevallig falen of quota opraakt. Stuurt een minimale
// prompt (geen artikel-JSON nodig) en geeft het ruwe antwoord + de
// tijdsduur terug, zodat meteen te zien is of de provider werkt én hoe
// snel die reageert.
const TEST_PROMPT_SYSTEM = "You are a helpful assistant used only to verify that an AI provider connection works correctly.";
const TEST_PROMPT_USER = "Reply with a single short, friendly sentence confirming you're working correctly.";

export async function testAiProvider(providerId) {
  const provider = getAllProviders().find((p) => p.id === providerId);
  if (!provider) throw new Error("Unknown provider");
  const cfg = getProviderConfig(providerId);
  if (!cfg?.api_key) throw new Error("No API key configured for this provider yet");

  const startedAt = Date.now();
  const rawText = await withTimeout(provider.call(cfg, TEST_PROMPT_SYSTEM, TEST_PROMPT_USER), PROVIDER_TIMEOUT_MS);
  // Een geslaagde test bewijst dat de provider weer bereikbaar is — een
  // eventuele oude cooldown (bijv. van een quota-fout een uur geleden) mag
  // dan meteen vervallen, in plaats van dat de normale generatie nog
  // moet wachten tot de 15 minuten vanzelf verlopen.
  providerCooldowns.delete(providerId);
  return { response: rawText.trim(), latencyMs: Date.now() - startedAt };
}
...
const VERIFICATION_PROMPT = `You are a strict fact-checking editor reviewing a colleague's draft article before
publication. You will be given the draft and the original source text(s) it was based on.

Check carefully:
- Does every factual claim, figure, name, date, and quote in the draft actually appear in the
  source text(s), literally or as a reasonable paraphrase? Flag anything that looks invented,
  exaggerated, or embellished beyond what the source actually supports.
- Is any important information from the source text missing that clearly should have been
  included?
- Does the draft contain any internal contradictions or unclear/confusing sentences?

If the draft holds up, return it completely UNCHANGED (identical title and body) and an empty
"issues_found" list. If you find real, concrete issues, return a corrected version that fixes
ONLY those specific issues — don't rewrite the style or tone, don't add anything not already
grounded in the source text(s), don't shorten or restructure parts that were already correct.

Respond ONLY with valid JSON, in this exact format, with no markdown code block around it:
{"title": "...", "body": "...", "issues_found": ["short description of issue 1", ...]}`;

// Optionele, losstaande tweede AI-aanroep die het concept nog eens tegen de
// brontekst aanhoudt vóórdat het bij de menselijke redacteur in de
// wachtrij komt — alleen actief als "Verification pass" aanstaat in
// Instellingen → Automatisering (kost een volledige extra AI-aanroep, dus
// bewust opt-in). Faalt deze stap zelf (bijv. alle providers tijdelijk
// onbereikbaar), dan valt terug op het ORIGINELE concept — een mislukte
// extra controle mag nooit de hele artikelgeneratie laten mislukken.
async function verifyDraft({ title, body, sourceText, additionalSources = [] }) {
  let combinedSource = sourceText;
  additionalSources.forEach((s, i) => {
    combinedSource += `\n\nSource ${i + 2} (${s.name || "unknown"}):\n${s.text}`;
  });
  const userPrompt = `Draft article:\nTitle: ${title}\nBody: ${body}\n\nOriginal source text(s):\n${combinedSource}`;

  try {
    const { rawText } = await callWithFallback(VERIFICATION_PROMPT, userPrompt);
    const parsed = parseDraftJSON(rawText);
    return {
      title: typeof parsed.title === "string" && parsed.title.trim() ? parsed.title : title,
      body: typeof parsed.body === "string" && parsed.body.trim() ? parsed.body : body,
      issues_found: Array.isArray(parsed.issues_found) ? parsed.issues_found.filter((i) => typeof i === "string") : [],
    };
  } catch {
    return { title, body, issues_found: [] };
  }
}
...
// Draait automatisch (geen aparte instelling — dit IS de manier waarop een
// ingesteld minimum wordt afgedwongen, niet slechts een suggestie in de
// prompt) zodra de eerste poging onder het ingestelde minimum blijft. Werkt
// uitsluitend met de brontekst zelf: geen nieuwe feiten, alleen meer
// context/achtergrond/uitleg van bestaande feiten, exact zoals de
// hoofdprompt dat ook voorschrijft. Precies één aanvul-poging — geen
// eindeloze cyclus als de brontekst simpelweg te dun is voor het
// ingestelde minimum; de bestaande body_too_short/content_thin-vlaggen
// blijven als laatste vangnet staan voor de menselijke redacteur.
async function expandDraftIfTooShort({ title, body, sourceText, additionalSources = [], minWordCount }) {
  if (countWords(body) >= minWordCount) return { title, body };

  let combinedSource = sourceText;
  additionalSources.forEach((s, i) => {
    combinedSource += `\n\nSource ${i + 2} (${s.name || "unknown"}):\n${s.text}`;
  });
  const expandPrompt = `You are a news editor. The draft article below is shorter than the required minimum of
${minWordCount} words. Expand it to at least ${minWordCount} words using ONLY the original source
text(s) below — add more context, background, explanation of significance, or elaboration on
details that are already present in the source text(s), but never invent new facts, figures,
quotes, or names that aren't in the source. If the source text genuinely doesn't contain enough
material to reach the minimum, expand as much as the source material honestly supports and
return that — do not pad with filler or repeat the same point in different words. Keep the
existing title and the parts of the body that already work; add to them rather than rewriting
from scratch.

Respond ONLY with valid JSON, in this exact format, with no markdown code block around it:
{"title": "...", "body": "..."}`;
  const userPrompt = `Draft article (too short):\nTitle: ${title}\nBody: ${body}\n\nOriginal source text(s):\n${combinedSource}`;

  try {
    const { rawText } = await callWithFallback(expandPrompt, userPrompt);
    const parsed = parseDraftJSON(rawText);
    return {
      title: typeof parsed.title === "string" && parsed.title.trim() ? parsed.title : title,
      body: typeof parsed.body === "string" && parsed.body.trim() ? parsed.body : body,
...
export async function generateDraft({ sourceText, sourceName, additionalSources = [] }) {
  ...
  const { min_word_count } = getAutomationSettings();
  const hasMultipleSources = additionalSources.length > 0;
  const systemPrompt = hasMultipleSources
    ? buildSystemPrompt(categoryNames, min_word_count) + buildFactCheckAddendum(categoryNames)
    : buildSystemPrompt(categoryNames, min_word_count);
  ...
  const { rawText, providerLabel, failedProviders } = await callWithFallback(systemPrompt, userPrompt);

  const parsed = parseDraftJSON(rawText);
  const combinedSourceText = [sourceText, ...additionalSources.map((s) => s.text)].join("\n");

  // Eerst proberen aan te vullen als het te kort is — VÓÓR de
  // body_too_short-weigeringscheck hieronder, anders krijgt een concept
  // van bijv. 30 woorden nooit de kans om aangevuld te worden en wordt het
  // altijd meteen afgewezen in plaats van gered. Bij een concept dat al
  // aan het minimum voldoet doet expandDraftIfTooShort niets (zie de
  // check aan het begin van die functie).
  let title = parsed.title;
  let body = parsed.body;
  const expanded = await expandDraftIfTooShort({ title, body, sourceText, additionalSources, minWordCount: min_word_count });
  title = expanded.title;
  body = expanded.body;

  // Vlaggen/confidence worden op het (mogelijk aangevulde) eindresultaat
  // berekend, niet op het oorspronkelijke concept — anders zou een terecht
  // aangevuld artikel toch nog als "te kort" gemarkeerd blijven staan.
  const flags = validateDraft({ sourceText: combinedSourceText, draft: { body } });
  const confidence_score = computeConfidence(flags);

  // Zelfs ná de aanvul-poging nog te kort (bijv. de brontekst was simpelweg
  // te dun, of de aanvul-aanroep zelf mislukte) — dan is dit hoogst-
  // waarschijnlijk een kapotte/afgekapte AI-respons, geen artikel dat
  // nuttig is voor de wachtrij. Zelfde behandeling als elke andere
  // generatiefout: overslaan, niet aanmaken.
  ...
  let ai_verification_notes = [];

  if (getAutomationSettings().verification_pass_enabled) {
    const verified = await verifyDraft({ title, body, sourceText, additionalSources });
    title = verified.title;
    body = verified.body;
    ai_verification_notes = verified.issues_found;
  }
  ...
  return {
    ...
    // Zichtbaar in het revisiescherm zodra een generatie pas na 1+
    // mislukte providers slaagde — anders is de enige aanwijzing dat er
    // iets misging de servertlogs, die je normaal nooit inziet.
    ai_provider_notes: failedProviders?.length > 0 ? failedProviders : [],
...
// One-time migration function: translates an existing Dutch article
// (title, body, and — critically — its "claims" / Key Facts) to English.
// Separate from translateArticle above (which now translates FROM English
// TO other languages, since the site itself became English) — this
// function is specific to scripts/migrate-translate-articles.mjs and can
// be removed once that migration is complete.
const TRANSLATE_TO_ENGLISH_PROMPT = `You are a professional translator. Translate the supplied
Dutch news article title and text into English, as naturally as possible for a native English
speaker, without adding, omitting, or changing facts. Preserve the paragraph structure (\\n
between paragraphs).
If a numbered list of "claims" is supplied, translate each one into English as well, keeping
the exact same order and the exact same number of items — these are short factual statements
extracted from the article, translate them the same way (no adding/omitting facts).
Respond ONLY with valid JSON in this exact format, with no markdown code block. Omit "claims"
entirely from your response if none were supplied:
{"title": "...", "body": "...", "claims": ["...", "..."]}`;

export async function translateArticleToEnglish({ title, body, claims }) {
  const claimTexts = Array.isArray(claims) ? claims.map((c) => c.text).filter(Boolean) : [];
  let userPrompt = `Title: ${title}\n\nText:\n${body}`;
  if (claimTexts.length > 0) {
    userPrompt += `\n\nClaims:\n${claimTexts.map((t, i) => `${i + 1}. ${t}`).join("\n")}`;
  }
  const { rawText } = await callWithFallback(TRANSLATE_TO_ENGLISH_PROMPT, userPrompt);
  const parsed = parseDraftJSON(rawText);
  if (!parsed.title || !parsed.body) {
    throw new Error("No valid translation received from the AI.");
  }

  let translatedClaims = claims;
  if (claimTexts.length > 0) {
    // Safety net: only accept the translated claims if the AI returned
    // exactly as many as were sent — otherwise we'd risk mismatching a
    // translated text with the wrong original "verified"/
    // "confirmed_by_sources" metadata. If the count doesn't match, keep
    // the original (Dutch) claims rather than risk corrupting the data —
    // this article just won't have its claims translated this round.
    if (Array.isArray(parsed.claims) && parsed.claims.length === claimTexts.length) {
      let ti = 0;
      translatedClaims = claims.map((c) =>
        c.text ? { ...c, text: parsed.claims[ti++] } : c
      );
    }
  }

  return { title: parsed.title, body: parsed.body, claims: translatedClaims };
}
...
function validateDraft({ sourceText, draft }) {
  ...
  // Ook expliciet op woordaantal checken, niet alleen letters — een
  // kort-maar-samenhangend "artikel" van bijv. 20 woorden bestaat uit
  // genoeg letters om de oude, puur letter-gebaseerde check te ontwijken,
  // maar is voor een lezer even onbruikbaar als echt kapotte/lege output.
  const body_too_short = letterCount < 100 || wordCount < 50;
...
function computeConfidence(flags) {
  ...
  // Was 0.1 — te zwak om een duidelijk te kort artikel (bijv. 20 van de
  // beoogde 350-500 woorden) zichtbaar te onderscheiden van een volwaardig
  // artikel; precies dit liet een veel te kort artikel eerder als 100%
  // confidence door de wachtrij glippen.
  if (flags.content_thin) score -= 0.25;

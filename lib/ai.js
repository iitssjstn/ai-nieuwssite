...
- Een artikel dat alleen de brontekst herformuleert is niet genoeg. Voeg altijd waarde toe door
  uit te leggen WAAROM dit relevant is en WAT het concreet betekent voor de lezer — nog steeds
  zonder nieuwe feiten te verzinnen, maar door bestaande feiten beter te duiden: leg vaktermen of
  onbekende namen/organisaties uit als de brontekst ze zonder context noemt, plaats een cijfer in
  perspectief (bijv. is dit veel of weinig vergeleken met wat gebruikelijk is, áls dat uit de
  brontekst is af te leiden), en maak concreet voor wie dit gevolgen heeft. Dit is wat een lezer
  niet al kreeg door alleen de kop te lezen.
...
const PROVIDERS = [
  ...
  },
  {
    id: "groq",
    label: "Groq",
    // llama-3.3-70b-versatile wordt door Groq uitgefaseerd per 16 augustus
    // 2026 — overgezet naar hun aanbevolen vervanger, openai/gpt-oss-120b.
    // Dit is een "redenerend" model — zonder reasoning_effort te beperken
    // kan het (gemeld, bekend probleem op Groq's eigen forum) zijn
    // denkstappen laten lekken in het antwoord, wat onze JSON-parsing
    // breekt. "low" houdt dat risico zo klein mogelijk.
    ...
    call: (cfg, systemPrompt, userPrompt) => {
      ...
      return callOpenAICompatible({
        ...
        systemPrompt,
        userPrompt,
        // Alleen relevant voor gpt-oss-modellen — bij een ander,
        // handmatig ingesteld Groq-model (dat dit mogelijk niet
        // ondersteunt) sturen we 'm niet mee.
...
export async function generateDraft({ sourceText, sourceName, additionalSources = [] }) {
  ...
  // Een kapot/afgekapt AI-antwoord (bijv. een body die alleen uit puntjes
  // bestaat) leidt niet tot een leeg concept in de wachtrij — dat is voor
  // niemand nuttig en moet toch handmatig weggeklikt worden. Behandelen we
  // hetzelfde als een andere generatiefout: overslaan, niet aanmaken.
  if (flags.body_too_short) {
    throw new Error("AI-antwoord bevatte geen bruikbare artikeltekst (mogelijk afgekapt of leeg antwoord)");
  }
...
function validateDraft({ sourceText, draft }) {
  ...
  // Vangt een kapot/afgekapt AI-antwoord op — bijv. een body die alleen uit
  // puntjes of andere leestekens bestaat, zonder daadwerkelijke inhoud.
  // Telt alleen letters (ook met accenten), niet cijfers/leestekens/spaties.
  const letterCount = ((draft.body || "").match(/\p{L}/gu) || []).length;
  const body_too_short = letterCount < 100;

  // Los van "kapot" (hierboven): een artikel dat wél inhoud heeft maar
  // duidelijk onder de beoogde 350-500 woorden blijft, is een risico voor
  // "content van weinig waarde" (bijv. bij een AdSense-beoordeling) — dit
  // blokkeert niets, maar geeft een zichtbare waarschuwing in de wachtrij
  // zodat een redacteur bewust kan kiezen om het artikel aan te vullen.

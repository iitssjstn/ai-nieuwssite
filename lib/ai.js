import { getProviderConfig, getCategories, getAutomationSettings } from "./db.js";
...
function getOrderedProviders() {
  ...
  if (!prioritize_speed) return PROVIDERS;
  const groq = PROVIDERS.find((p) => p.id === "groq");
  if (!groq) return PROVIDERS;
  return [groq, ...PROVIDERS.filter((p) => p.id !== "groq")];
}

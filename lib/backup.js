...
function ensureBackupDir() {
  try {
    ...
    if (err.code === "EACCES" || err.code === "EPERM") {
      throw new Error(
        `Geen schrijfrechten voor de back-up-map (${BACKUP_DIR}). Draai op de server ` +
...
// Stuurt een al lokaal gemaakte back-up door naar de externe ontvanger
// (de losse "backup-receiver"-applicatie op een tweede server). Bewust
// best-effort: als dit mislukt (server niet bereikbaar, verkeerd
// wachtwoord, etc.) gooien we geen fout — de lokale back-up is en blijft
// hoe dan ook gelukt, dit is puur een extra kopie.
export async function pushBackupToRemote(filename, { url, key }) {
  if (!url || !key) return { success: false, skipped: true };

  const fullPath = path.join(BACKUP_DIR, filename);
  if (!fs.existsSync(fullPath)) return { success: false, error: "Lokale back-up niet gevonden" };

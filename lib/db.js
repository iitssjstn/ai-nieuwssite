...
// AdMaven's site-ownership verification tag — a plain <meta> tag, not a
// script, so (unlike the ad scripts in ConsentGatedScripts) it's rendered
// unconditionally in <head> rather than gated behind cookie consent: it
// carries no tracking or executable code of its own.
export function getAdMavenPlacementId() {
  const data = read();
  return data.settings?.admaven_placement_id || null;
}

export function setAdMavenPlacementId(id) {
  const data = read();
  data.settings = data.settings || {};
  data.settings.admaven_placement_id = id || null;
  write(data);
}

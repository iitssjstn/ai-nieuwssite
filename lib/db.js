...
export function setSiteSettings({ site_name, site_description, favicon_url, google_site_verification, bing_site_verification }) {
  const data = read();
  data.settings = data.settings || {};
...
export function getAdSlots() {
  ...
  return {
    ...
    banners: {
      ...
      category_left: data.settings?.ad_slots?.banners?.category_left || null,
      category_right: data.settings?.ad_slots?.banners?.category_right || null,
    },
  };
}

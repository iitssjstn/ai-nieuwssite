...
// Bewust een EIGEN, apart bestand — niet in db.json zelf. De RSS-planner
// draait als apart proces naast de hoofdserver, allebei met hun eigen
// in-memory kopie van db.json; als beide rond hetzelfde moment een
// read-wijzig-schrijf op datzelfde grote bestand doen (bijv. de planner
// werkt zijn hartslag bij, terwijl jij tegelijk een instelling opslaat),
// kan de ene schrijfactie de andere overschrijven — precies zo verdween
// een hartslag-update eerder na een instellingenwijziging. Dit bestandje
// bevat NOOIT iets anders dan de hartslag zelf, dus een gemiste
// schrijfactie hier kan ook nooit iets anders beschadigen dan zichzelf,
// en herstelt vanzelf binnen een minuut bij de eerstvolgende tick.
const HEARTBEAT_PATH = path.join(process.cwd(), "data", "scheduler-heartbeat.json");
// Zelfde reden als hierboven bij de hartslag, maar dan voor een nog veel
// vaker geraakt pad: incrementPageview()/incrementCountry() vuren af bij
// LETTERLIJK ELKE paginaweergave op de hele publieke site. Die deden
// voorheen elk een volledige, synchrone herschrijving van de complete
// database (incl. alle artikelen en revisies) voor het ophogen van één
// simpel getal — dat blokkeert tijdens die herschrijving tijdelijk de
// HELE server (Node.js is single-threaded) voor ALLE gelijktijdige
// bezoekers, niet alleen degene die de view veroorzaakte. Nu raakt dit
// alleen nog een klein, apart bestandje.
const SITE_STATS_PATH = path.join(process.cwd(), "data", "site-stats.json");
...
function write(data) {
  // Zonder pretty-printing (geen ", null, 2") — bespaart zowel
  // serialisatietijd als bestandsgrootte bij elke schrijfactie. Niets in
  // de code leest dit bestand ooit anders dan via JSON.parse(), dus de
  // opmaak zelf heeft geen enkele functionele betekenis — puur
  // leesbaarheid voor een mens die het bestand handmatig zou openen, wat
  // hier nooit de bedoeling is.
...
export function updateSchedulerHeartbeat() {
  // Rechtstreeks een klein, eigen bestandje schrijven — GEEN read() van het
  // grote db.json ervoor, precies om de race condition met de hoofdserver
  // te vermijden die eerder deze exacte update liet verdwijnen.
  fs.writeFileSync(HEARTBEAT_PATH, JSON.stringify({ updated_at: new Date().toISOString() }));
}

export function getSchedulerHeartbeat() {
  try {
    const raw = fs.readFileSync(HEARTBEAT_PATH, "utf-8");
    return JSON.parse(raw).updated_at || null;
  } catch {
    return null; // bestand bestaat nog niet (planner nog nooit gedraaid) — geen fout, gewoon "nog nooit"
  }
}
...
export function getTrendingTags({ days = 7, limit = 5 } = {}) {
  ...
  // Alleen articles (zonder revisions) + article_views_by_day klonen i.p.v.
  // de hele database — dit draait op elke homepage-weergave, zelfde reden
  // als bij getArticles() hierboven.
  const articles = structuredClone(lightArticlesCache);
  const byDay = structuredClone(cache.article_views_by_day || {});
...
export function getPolls() {
  ensureCacheFresh();
  return structuredClone(cache.polls || []);
}
...
// Eenmalige, automatische migratie: als het nieuwe, aparte statistiek-
// bestand nog niet bestaat maar de hoofddatabase nog wél oude pageviews/
// visitor_countries bevat (van vóór deze wijziging), wordt die data
// overgenomen — zodat bestaande statistiekgeschiedenis niet verloren gaat
// bij de overstap naar dit lichtere opslagmechanisme. Draait maar één
// keer: zodra site-stats.json bestaat, wordt dit pad nooit meer genomen.
function readSiteStats() {
  try {
    return JSON.parse(fs.readFileSync(SITE_STATS_PATH, "utf-8"));
  } catch {
    const data = read();
    const migrated = {
      pageviews: data.pageviews || {},
      visitor_countries: data.visitor_countries || {},
    };
    writeSiteStats(migrated);
    return migrated;
  }
}

function writeSiteStats(stats) {
  fs.writeFileSync(SITE_STATS_PATH, JSON.stringify(stats));
}
...
export function incrementPageview() {
  ...
  stats.pageviews[day] = (stats.pageviews[day] || 0) + 1;
...
export function getPageviewStats(days = 14) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    d.setDate(d.getDate() - i);
    const key = getLocalDateKey(d);
    result.push({ date: key, views: stats.pageviews?.[key] || 0 });
  }
  return result;
}

// Puur geaggregeerde tellingen per land — NOOIT een IP-adres of iets
// anders herleidbaars naar een individuele bezoeker opgeslagen, alleen een
// oplopend totaal per landcode (zelfde soort simpele teller als
// pageviews hierboven). Uitsluitend bedoeld om te zien uit welke landen
// bezoekers komen, bijv. voor advertentieverkoop.
export function incrementCountry(countryCode) {
  if (!countryCode) return;
  const stats = readSiteStats();
  stats.visitor_countries = stats.visitor_countries || {};
  stats.visitor_countries[countryCode] = (stats.visitor_countries[countryCode] || 0) + 1;
  writeSiteStats(stats);
}

export function getCountryStats() {
  return readSiteStats().visitor_countries || {};
}

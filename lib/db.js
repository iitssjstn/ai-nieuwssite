...
export function incrementCountry(countryCode) {
  ...
  const data = read();
  data.visitor_countries = data.visitor_countries || {};
  data.visitor_countries[countryCode] = (data.visitor_countries[countryCode] || 0) + 1;
  write(data);
}

export function getCountryStats() {
  ensureCacheFresh();
  return structuredClone(cache.visitor_countries || {});
}

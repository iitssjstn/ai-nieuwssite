...
export function updateAdSubmission(id, updates) {
  ...
  // Slechts één actieve advertentie per plek tegelijk — zodra deze wordt
  // goedgekeurd, wordt elke andere al goedgekeurde advertentie voor
  // dezelfde plek automatisch vervangen, zodat een plek nooit twee
  // advertenties tegelijk kan claimen.
  if (updates.status === "approved") {
    for (const other of data.ad_submissions) {
      if (other.id !== id && other.slot === sub.slot && other.status === "approved") {
...
export function getActiveAdForSlot(slotName) {
  ensureCacheFresh();
  const sub = (cache.ad_submissions || []).find((s) => s.slot === slotName && s.status === "approved");

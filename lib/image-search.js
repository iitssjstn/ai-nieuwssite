...
export async function searchStockPhoto(query, providerConfigs) {
  ...
  if (attempted.length === 0) {
    // Geen enkele provider ingesteld — dit is de enige situatie waarin we
    // stil null teruggeven, want dit hoort de automatische flow (bij het
    // genereren van een concept) nooit te blokkeren.
    return null;
  }

  // Wél iets geprobeerd, maar niets gevonden/gelukt — de reden meegeven i.p.v.
  // hem te verzwijgen, zodat je bij een storing of rate-limit ook echt ziet
  // wát er misging in plaats van een stil "het werkt niet meer".

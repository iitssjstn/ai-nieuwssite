...
async function fetchPexelsResults(query, apiKey) {
  ...
  return (data.photos || []).map((photo) => ({
    ...
    source: "Pexels",
    // Pexels' own short description of what's actually in the photo —
    // more accurate for image-search alt text than reusing the article
    // headline, since the photo is generic stock footage, not a picture
    // of the specific news event.
    alt: photo.alt || null,
  }));
}
...
async function fetchPixabayResults(query, apiKey) {
  ...
  return (data.hits || []).map((photo) => ({
    ...
    source: "Pixabay",
    // Pixabay has no free-text description field, only comma-separated
    // tags — still more descriptive of the actual image than the
    // unrelated news headline.
    alt: photo.tags ? photo.tags.split(",").map((t) => t.trim()).join(", ") : null,
  }));
}

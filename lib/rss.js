...
// Bewust op 300px gezet (was 600px): veel Nederlandse nieuwsfeeds leveren
// in hun RSS-feed alleen kleine voorvertoning-thumbnails, dus een hogere
// drempel zou vrijwel nooit een bron-afbeelding accepteren.
...
function extractSourceImage(item) {
  ...
  // Sommige uitgevers nesten media:content binnen een media:group-element
  // i.p.v. rechtstreeks op het item — en dat is dan een array.
  ...
  if (groupContentUrl) return groupContentUrl;
  // Fallback: sommige uitgevers gebruiken geen aparte media/enclosure-tag,
  // maar zetten de afbeelding als gewone <img>-tag in de HTML-inhoud zelf
  // (content:encoded). Pakt de eerste <img> die we daar vinden.

...
import {
  ...
  getAllImageProviderConfigs, getCustomImageProviders, getAutomationSettings, getSiteSettings, markSourcePolled,
} from "./db.js";
import { generateDraft, generateUpdatedDraft } from "./ai.js";
import { pingIndexNow, submitUrlToBing } from "./indexnow.js";

// Alleen bruikbaar als de admin de publieke site-URL heeft ingevuld op
// Instellingen → SEO — deze planner draait als los achtergrondproces
// (scripts/rss-scheduler.mjs) zonder inkomend HTTP-request, dus er is hier
// geen host-header om anders uit af te leiden.
function pingSearchEnginesForArticle(slug) {
  const { site_url } = getSiteSettings();
  if (!site_url) return;
  const url = `${site_url}/artikel/${slug}`;
  pingIndexNow(site_url, url);
  submitUrlToBing(site_url, url);
}
  ...
  for (const item of newItems) {
    ...
    try {
      ...
      if (related && related.status === "pending_review" && auto_gather_sources) {
        ...
        if (existing) {
          ...
          updateArticle(existing.id, {
            ...
            ai_provider_notes: draft.ai_provider_notes || [],
      ...
      if (related && related.status === "published" && auto_update_published) {
        ...
        if (existing) {
          ...
          if (passesAutoUpdateGate(updateResult, auto_update_min_confidence)) {
            ...
            pingSearchEnginesForArticle(updatedArticle.slug);
      ...
      if (shouldAutoPublish) {
        ...
        pingSearchEnginesForArticle(article.slug);
      }
